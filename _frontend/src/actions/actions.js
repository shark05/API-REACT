...
import {
  ...
  DOACOES_LIST_SET_PAGE,
  DOACOES_FORM_UNLOAD,
  DOACOES_UNLOAD,

  MENSALIDADES_LIST_ERROR,
  MENSALIDADES_LIST_RECEIVED,
  MENSALIDADES_LIST_REQUEST,
  MENSALIDADES_LIST_SET_PAGE,
...
export const doacoesListRequest = () => ({
  type: DOACOES_LIST_REQUEST,
});

export const doacoesListError = (error) => ({
  type: DOACOES_LIST_ERROR,
  error
});

export const doacoesListReceived = (data) => ({
  type: DOACOES_LIST_RECEIVED,
  data
});

export const doacoesListSetPage = (page) => ({
  type: DOACOES_LIST_SET_PAGE,
  page
});

export const doacoesListFetch = (page = 1) => {
  return (dispatch) => {
    dispatch(doacoesListRequest());
    //return requests.get(`/doacoes?_page=${page}`)
    return requests.get(`/doacoes`)
      .then(response => dispatch(doacoesListReceived(response)))
      .catch(error => dispatch(doacoesListError(error)));
  }
};


export const mensalidadesListRequest = () => ({
  type: MENSALIDADES_LIST_REQUEST,
});

export const mensalidadesListError = (error) => ({
  type: MENSALIDADES_LIST_ERROR,
  error
});

export const mensalidadesListReceived = (data) => ({
  type: MENSALIDADES_LIST_RECEIVED,
  data
});

export const mensalidadesListSetPage = (page) => ({
  type: MENSALIDADES_LIST_SET_PAGE,
  page
});

export const mensalidadesListFetch = (page = 1) => {
  return (dispatch) => {
    dispatch(mensalidadesListRequest());
    return requests.get(`/doacoes?_page=${page}`)
      .then(response => dispatch(mensalidadesListReceived(response)))
      .catch(error => dispatch(mensalidadesListError(error)));
  }
};

export const doacoesAdd = (valor, formapagamento) => {
  return (dispatch) => {
    return requests.post(
      ...
      if (401 === error.response.status) {
        ...
        throw new SubmissionError({
          _error: 'Você não tem permissão de salvar esta doação!'
        });
      }
...
export const doacoesFormUnload = () => ({
  type: DOACOES_FORM_UNLOAD
});
...
export const userRegister = (name, password, email) => {
  return (dispatch) => {
    console.log(email);
    return requests.post('/users', {name, email, password}, false)
